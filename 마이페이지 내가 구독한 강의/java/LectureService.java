...
import com.lms.urbangreen.urbangreenproject.lecture.repository.LectureRepo;
...
public class LectureService {
    ...
    // 사용자가 구독한 강의 목록을 DTO로 변환
    public List<LectureListResponseDto> getSubscribedLectureDtos(int userId) {
        // 1. 구독된 강의 엔티티 리스트를 가져옵니다.
        List<Lecture> subscribedLectureList = lectureRepo.findSubscribedLecturesByUserId(userId);

        // 2. 각 강의 엔티티를 DTO로 변환하면서 강사 닉네임을 조회합니다.
        return subscribedLectureList.stream()
                .map(lecture -> {
                    String nickname = userService.findNicknameByUserId(lecture.getUser_id())
                            .orElse("알 수 없는 강사");
                    return new LectureListResponseDto(lecture, nickname);
