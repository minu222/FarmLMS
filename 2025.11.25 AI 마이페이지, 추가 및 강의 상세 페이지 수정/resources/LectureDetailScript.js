...
// 퀴즈 100% 달성 알림이 이미 떴는지 비디오 ID별로 체크
let isQuizAlertShown = {};

// === [추가된 변수] ===
let isVideoSeekable = false; // 현재 비디오의 탐색 가능 여부 상태
// ======================
...
/**
 * 탐색 기능(스크러빙)을 동적으로 제어합니다.
 * @param {boolean} enable - true면 탐색 가능, false면 탐색 불가
 */
function toggleSeekability(enable) {
    if (!videoJsPlayer) return;

    // Video.js 플레이어 DOM 요소
    const playerEl = videoJsPlayer.el();
    // 진행 표시줄(Progress Control) 요소
    const progressControl = playerEl.querySelector('.vjs-progress-control');

    if (progressControl) {
        if (enable) {
            // 탐색 허용: 마우스 이벤트 활성화 및 커서 포인터
            progressControl.style.pointerEvents = 'auto';
            progressControl.style.cursor = 'pointer';
            isVideoSeekable = true;
            console.log("탐색 기능이 해제되었습니다. 모든 지점 이동 가능.");
        } else {
            // 탐색 불가: 마우스 이벤트 무시 및 커서 기본값
            progressControl.style.pointerEvents = 'none';
            progressControl.style.cursor = 'default';
            isVideoSeekable = false;
            console.log("탐색 기능이 잠금되었습니다. 시청한 진도까지만 이동 가능.");
        }
    }
}
...
function initializeVideoEvents(videoData) {
    ...
    // === [수정/추가 1] 초기 탐색 가능 여부 설정 ===
    if (progressPercentage === 100) {
        // 이미 100%인 경우 탐색 기능 해제 (자유 이동 허용)
        toggleSeekability(true);
    } else {
        // 100% 미만인 경우 탐색 기능 잠금 (시청 진도까지만 허용)
        toggleSeekability(false);
    }
    // ===========================================
    ...
    fetch(`/api/video/${videoId}/progress`, {
        ...
        .then(response => {
            if (!response.ok) {
                ...
                // === [핵심 수정: 100% 달성 시 탐색 해제] ===
                if (progressPercentage === 100) {
                    if (!isQuizAlertShown[videoId]) {
                        alert("강의를 100% 시청했습니다. 이제 퀴즈를 풀 수 있으며, 영상을 자유롭게 이동할 수 있습니다.");
                        isQuizAlertShown[videoId] = true;
                    }
                    // 탐색 기능 해제 (스크러빙 허용)
                    toggleSeekability(true);
                } else if (progressPercentage < 100) {
                    // 100% 미만일 경우 혹시나 해제되어 있다면 다시 잠금 (안전장치)
                    toggleSeekability(false);
                }
                // ==========================================
            }
...
function loadVideo(element) {
    ...
    fetch(`/api/video/${videoId}`)
        ...
        .then(videoData => {
            ...
            // 2. 이벤트 초기화 (이 안에서 진도율에 따라 탐색 가능 여부 결정됨)
...
function startQuiz() {
    ...
    if (!currentVideoData || !currentVideoData.video_id || !currentVideoData.video_time || currentVideoData.video_time <= 0) {
    ...
    // ----------------------------------------------------
    // 4. 팝업 크기 및 위치 계산 로직 (수정된 부분)
    // ----------------------------------------------------

    // 화면 크기의 60%를 팝업 크기로 설정
    const screenWidth = window.screen.width;
    const screenHeight = window.screen.height;

    const popupWidth = Math.round(screenWidth * 0.6); // 화면 너비의 60%
    const popupHeight = Math.round(screenHeight * 0.6); // 화면 높이의 60%

    // 팝업을 화면 중앙에 위치시키기 위한 좌표 계산
    const popupLeft = Math.round((screenWidth - popupWidth) / 2);
    const popupTop = Math.round((screenHeight - popupHeight) / 2);

    // 팝업 URL 및 옵션 문자열 생성
    const quizUrl = `/quiz/view?videoId=${currentVideoId}`;
    ...
    // 5. 퀴즈 페이지로 이동 (팝업/새 탭)
    window.open(quizUrl, 'QuizWindow', windowFeatures);
}