package com.lms.urbangreen.urbangreenproject.user.service;
...
import com.lms.urbangreen.urbangreenproject.user.repository.UserRepository;
...
public class UserService {
    ...
    // 아이디 + 이메일로 유저 조회 (비밀번호 찾기 1단계)
    public User findUserForPwReset(String id, String email) {
        return repository.findByIdAndEmail(id, email);
    }

    // 현재 비밀번호 없이 ID만으로 새 비밀번호 설정 (비밀번호 찾기 2단계)
    public PwChangeResult resetPasswordById(String id, String newPw) {
        User dbUser = repository.findById(id);
        if (dbUser == null) {
            return new PwChangeResult(false, "사용자를 찾을 수 없습니다.");
        }
        if (!isValidPassword(newPw)) {
            return new PwChangeResult(false, "새 비밀번호는 영문과 숫자를 포함한 8자 이상이어야 합니다.");
        }
