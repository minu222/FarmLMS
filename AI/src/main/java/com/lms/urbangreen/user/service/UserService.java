package com.lms.urbangreen.user.service;

import com.lms.urbangreen.user.entity.User;
import com.lms.urbangreen.user.entity.UserType;
import com.lms.urbangreen.user.repository.UserRepository;
...
public class UserService {
    ...
    // id로 닉네임 조회 (강의 작업)
    public Optional<String> findNicknameByUserId(int userId) {
        String sql = "SELECT nickname FROM all_users WHERE user_id = ?";
        try {
            // queryForObject를 사용하되, 결과가 없을 경우를 대비하여 Optional을 반환
            String nickname = jdbcTemplate.queryForObject(sql, String.class, userId);
    ...
    // 강사 판단 (qna)
    public boolean isUserInstructor(int userId) {
        String sql = "SELECT user_type FROM all_users WHERE user_id = ?";
        try {
            // DB에서 user_type을 문자열로 조회합니다.
            String userTypeStr = jdbcTemplate.queryForObject(sql, String.class, userId);

            // 조회된 문자열을 UserType Enum으로 변환합니다.
            UserType userType = UserType.valueOf(userTypeStr.toLowerCase());

            // 강사 타입인지 확인합니다.
            return userType.toString().equalsIgnoreCase("TEACHER");

        } catch (EmptyResultDataAccessException e) {
            // user_id가 DB에 없는 경우
            return false;
        } catch (IllegalArgumentException e) {
            // user_type 문자열이 UserType Enum에 매핑되지 않는 경우 (데이터 오류)
            System.err.println("UserType 매핑 오류: " + e.getMessage());
            return false;
        }
    }
